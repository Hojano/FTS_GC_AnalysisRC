...
def integrate_peaks(frame, Peaks,Peaks_TCD,Peaks_channel1):
    df = frame['FID_M'] #only look into second channel
    integrals_list = []
    for i in range(0,len(Peaks)):
        #ideantify peaks and valleys in the range of the peak
        integral = integrate.trapz(df[(df.index > Peaks[i][1][0]) & (df.index < Peaks[i][1][1])])
...
def parse_log(logfile_path):
    ...
    Log = Log[['Date/Time', 'Oven PV', 'MFC Ar pv','MFC CO pv','MFC H2 pv','MFC He pv','MFC O2 pv', 'MFC N2 pv', 'Pressure R1']]
    Log['timestamp'] = pd.to_datetime(Log['Date/Time'], format='%d-%b-%Y %H:%M:%S', errors='coerce')
    return Log
...
def get_peaknames(Peaks_channel1,Peaks_channel2,Peaks_TCD):
    ...
    for i in range(0,len(Pameeakns_TCD)):
